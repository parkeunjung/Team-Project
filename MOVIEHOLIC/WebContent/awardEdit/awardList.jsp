...
<jsp:include page="../Share/manage_Header.jsp"/>
	...
	<%
		...
		// awards 테이블: award 수 페이지수 개산
		rs = stmt.executeQuery("SELECT COUNT(*) FROM awards");
		rs.next();
		...
		stmt = conn.createStatement();
		rs = stmt
				.executeQuery("SELECT * FROM awards ORDER BY id LIMIT "
						+ startPos + ", " + numInPage);
	%>
	...
	<div class="list">
		...
		<nav class="pagination_centered">
			...
			<tr>
				...
				%>
				<td><a href="awardList.jsp?page=<%=pageNo - 1%>">&laquo;</a></td>
				<%
					}
						...
						for (int i = startPageNo; i <= endPageNo; i++) {
							...
							out.println("<a href='awardList.jsp?page=" + i + "'>" + i
									+ "</a>");
							out.println("</td>");
						}
				...
				%>
				<td><a href="awardList.jsp?page=<%=pageNo + 1%>">&raquo;</a></td>
				<%
					}
				%>
				</tr>
		...
		<div class="list_option">
			<a href="awardWrite.jsp" class="btn btn-primary">글쓰기</a>
		</div>
	</nav>
	</div>
</div>
	...
	$(function() {
		$("a[data-action='delete']").click(function() {
			if (confirm("정말로 삭제하시겠습니까?")) {
				location = 'awardDelete.jsp?id=' + $(this).attr('data-id');
			}
			return false;
		});
	});
